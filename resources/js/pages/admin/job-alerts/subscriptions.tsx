...
export default function Subscriptions({ subscriptions, filters, locations, practiceAreas }: Props) {
    const [search, setSearch] = useState(filters.search || '');
    const [selectedIds, setSelectedIds] = useState<number[]>([]);

    const handleSearch: FormEventHandler = (e) => {
        e.preventDefault();
        router.get('/admin/job-alerts/subscriptions' + queryParams({ query: { ...filters, search, page: 1 } }), {}, { preserveState: true });
    };
    ...
    const handleFilter = (key: string, value: string) => {
        ...
        router.get('/admin/job-alerts/subscriptions' + queryParams({ query: newFilters }), {}, { preserveState: true });
    };

    const handleSort = (sortBy: string) => {
        const sortOrder = filters.sort_by === sortBy && filters.sort_order === 'asc' ? 'desc' : 'asc';
        router.get('/admin/job-alerts/subscriptions' + queryParams({ query: { ...filters, sort_by: sortBy, sort_order: sortOrder } }), {}, { preserveState: true });
    };
    ...
    return (
        <>
            <Head title="Job Alert Subscriptions - Admin" />

            <div className="space-y-6">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-3xl font-bold">Job Alert Subscriptions</h1>
                        <p className="mt-2 text-muted-foreground">Manage user subscriptions and operational actions</p>
                    </div>
                    <Link
                                        ...
                                        <TableRow key={subscription.id}>
                                            ...
                                            <TableCell className="text-right">
                                                <div className="flex justify-end gap-2">
                                                    <Button size="sm" variant="ghost" onClick={() => handleToggle(subscription.id, subscription.is_active)}>
                                                        {subscription.is_active ? <ToggleLeft className="h-4 w-4" /> : <ToggleRight className="h-4 w-4" />}
                                                    </Button>
...
Subscriptions.layout = (page: React.ReactNode) => <AdminLayout>{page}</AdminLayout>;
